...
static uint32_t get_crc32_and_length_gg(FILE *f, uint64_t *length);
...
static uint32_t calc_hash(char *s, int n, uint32_t v) {
    for (int i = 0; i < n; ++i) {
        v ^= (unsigned char)s[i];
        v *= 0x1000193;
        v &= 0xffffffff;
...
static uint32_t get_new_crc32(char *buffer, int n, uint32_t crc) {
...
int main(int argc, char **argv) {
    FILE *f = fopen("./libZapus.so", "r+b");
	if (!f) {
		printf("I/O error: fopen\n");
        exit(-1);
    }
    uint64_t len;
    uint32_t init_crc = get_crc32_and_length(f, &len);
    uint32_t checksum = 0x614C5347;
    uint32_t newcrc = reverse_bits(0x614C5347);
    uint32_t init_v = 0xe7c07254;

    char buf[0x10];
    uint32_t *pad = (uint32_t*)buf;
    uint32_t *calc = (uint32_t*)(buf + 4);

    for (int j = 0; j < FORK_CNT; ++j) {
        int pid = fork();
        if (pid) {
        ...
        for (uint32_t i = j; i < 4294967200U; i += FORK_CNT) {
            *pad = i;
            *calc = 0;
            uint32_t crc = get_new_crc32(buf, 8, init_crc);
            // printf("%#x\n", reverse_bits(crc));
            // Compute the change to make
            uint32_t delta = crc ^ newcrc;
            delta = (uint32_t)multiply_mod(reciprocal_mod(pow_mod(2, 4 * 8)), delta);
            *calc ^= reverse_bits(delta);
            uint32_t v = calc_hash(buf, 8, init_v);
            if (v == checksum) {
                printf("%u\n", i);
            }
            // printf("%#x %#x\n", *calc, v);
...
// 	newcrc = reverse_bits(newcrc);
// 	const char *path = "/data/local/tmp/libZapus.so";
...
// 	// fclose(fp);
// 	// printf("%llu\n", size);
// 	const char* out=modify_file_crc32(path, size, newcrc, true);
...
const char *modify_file_crc32(const char *path, uint64_t offset, uint32_t newcrc, bool printstatus) {
	...
	uint64_t length;
	uint32_t crc = get_crc32_and_length_gg(f, &length);
	// length += 4;
	// printf("fuck: %llu\n", length);
	...
	// Compute the change to make
	uint32_t delta = crc ^ newcrc;
	// printf("$$$ %u %u %u %u\n", crc, newcrc, delta, reverse_bits(crc));
	// printf("$$$  %llu %u\n", reciprocal_mod(pow_mod(2, (length - offset) * 8)), delta);
	// printf("$$$  %llu\n", multiply_mod(reciprocal_mod(pow_mod(2, (length - offset) * 8)), delta));
	...
	*length = 0;
	...
	while (true) {
		...
		if (flag) {
			...
			n = fread(buffer, sizeof(buffer[0]), sizeof(buffer) / sizeof(buffer[0]), f);
			if (ferror(f) != 0) {
				perror("fread");
				exit(EXIT_FAILURE);
			}
