...
void gdb(uint64_t a,uint64_t b,uint64_t c){
    ...
    hit = 1;
    p = locate_call(p);
    next_addr = (*(uint32_t*)(p + 1) + (uint64_t)(p + 5))&0xffffffff;
...
int main(int argc,char* argv[],char*envp[]){
    loadfile(argv[1]);
    uint64_t off = atoi(argv[2]);
    prog_init();
    //next_addr = (void (*)(uint8_t))(0x190D03+ELF_ADD);
    next_addr = (void (*)(uint8_t))(off+ELF_ADD);
    for(int i=0;i<3;i++){
        for(b=0;!hit &&b<0x100;b++){
            next_addr(b&0xff);
        }
        data[i] = --b;
        hit = 0;
